...
from sstg_explorer.baselines import (
    BaseExplorer,
    UniformGridExplorer,
    RRTExplorer,
    FrontierExplorer,
...
class BenchmarkRunner:
    ...
    def create_algorithm(self, algorithm_name: str, **kwargs) -> BaseExplorer:
        ...
        if algorithm_name == 'uniform_grid':
            ...
            return explorer
        elif algorithm_name in ('sstg_explorer', 'sstg_optimal'):
            # Final SSTG-Explorer configuration selected by the ablation study.
            # ``sstg_optimal`` remains as a compatibility alias for old results.
            config = ExplorerConfig(
                frontier_strategy=FrontierSelectionStrategy.ENHANCED_DISTANCE,
                use_astar=True,
